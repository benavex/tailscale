...
//
// The caller is responsible for installing a PeerByIPPacketFunc on the device
// for outbound packet routing (e.g. via Engine.SetPeerByIPPacketFunc).
func ReconfigDevice(d *device.Device, cfg *Config, logf logger.Logf) (err error) {
	...
	if err := d.SetPrivateKey(key.NodePrivateAs[device.NoisePrivateKey](cfg.PrivateKey)); err != nil {
		return err
	}
